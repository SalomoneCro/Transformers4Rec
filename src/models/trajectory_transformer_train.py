...
def _resolve_paths() -> Tuple[str, str, str, str]:
    ...
    model_path = os.path.join(project_root, "models", "modelo_trajectory_transformer.pth")
...
class TrajectoryTransformer(nn.Module):
    ...
    def _causal_mask(self, seq_len: int, device: torch.device) -> torch.Tensor:
        # bool mask: True = posición bloqueada (evita warning por tipos mezclados)
        return torch.triu(
...
def main() -> None:
    ...
    print(f"🚀 Entrenando Trajectory Transformer por {epochs} épocas...")
    ...
    tt_metrics = topk_metrics_from_logits(test_logits, test_targets, topk)
    ...
    print("\n=== Evaluación Test ===")
    print(format_metrics("TrajectoryTransformer", tt_metrics, topk))
